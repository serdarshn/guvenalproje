...
export default function AddSparePartPage() {
  ...
  const generateSlug = (name: string) => {
    return name
      ...
      .replace(/[^a-z0-9-]+/g, ''); // Geçersiz karakterleri kaldır
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append('type', 'spare');
      ...
      if (data.success) {
        ...
        throw new Error(data.message || 'Yedek parça eklenirken bir hata oluştu');
      }
    } catch (error) {
      console.error('Yedek parça ekleme hatası:', error);
      setError('Yedek parça eklenirken bir hata oluştu. Lütfen tekrar deneyin.');
      toast.error('Yedek parça eklenirken bir hata oluştu!');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <p className="text-sm text-gray-500">Tüm Yedek Parçalar</p>
          </div>
